    ...
    if (
      confirm(
        "Are you sure you want to submit? The following issues were detected:\n" +
          (testsModifiedWithoutSaving ? testsModifiedMessage + "\n" : "") +
          (isBotResponseInvalid() ? botResponseMessage + "\n" : "") +
          (isAlignmentScoreLow() ? alignmentScoreMessage + "\n" : "") +
...
const isPython = () => {
  ...
  const hasPythonInParagraphs = Array.from(document.querySelectorAll("p")).some(
    (p) => p.textContent.includes("Python")
  );
